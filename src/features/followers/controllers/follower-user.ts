...
export class Add {
  public async follower(req: Request, res: Response): Promise<void> {
    ...
    const followersCache: Promise<void> = followerCache.updateFollowerCountInCache(`${followerId}`, 'followersCount', 1);
    const followeeCache: Promise<void> = followerCache.updateFollowerCountInCache(`${req.currentUser!.userId}`, 'followingCount', 1);
    await Promise.all([followersCache, followeeCache]);
